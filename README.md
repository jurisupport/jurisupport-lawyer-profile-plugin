...
- Git
- Claude Code 설치 및 로그인 완료 상태
- JuriSupport 연동 실행 시 사용할 JuriSupport MCP 토큰

## Claude Code 설치 확인

먼저 Claude Code가 설치되어 있는지 확인하고, 없으면 설치합니다.
...
처음 설치했다면 새 터미널에서 `claude`를 실행해 로그인합니다.

JuriSupport 전체 플러그인을 함께 설치하면 사건자료·법률자료 도구를 더 많이 써서 강점찾기가 더 정확해집니다. 원하지 않으면 건너뛰고 Claude Code만 사용해도 됩니다.
설치되어 있으면 강점찾기 플러그인이 사건기록·작성서류 동기화와 사건 인덱스를 먼저 활용해 반복 업무 패턴을 잡습니다.
...
read -r answer
case "$answer" in
  [nN]|[nN][oO]) echo "Claude Code만 사용합니다." ;;
...
```powershell
$answer = Read-Host "JuriSupport 전체 플러그인도 설치할까요? 설치하면 강점찾기가 더 정확해집니다. [Y/n]"
if ($answer -match '^(n|N|no|NO)$') { "Claude Code만 사용합니다." } else { irm https://raw.githubusercontent.com/jurisupport/jurisupport-plugins/main/windows-bootstrap.ps1 | iex }
```
