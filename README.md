...
완성된 프로필은 자기소개 정리, 홈페이지·블로그 소개글 초안, 상담 분야 점검, 앞으로 받고 싶은 사건 방향 정리에 활용할 수 있습니다. JuriSupport에 올리지 않는 경우에도 브라우저에서 바로 고쳐 쓸 수 있는 HTML 파일로 받을 수 있고, 그 안에는 나중에 JuriSupport에 올릴 수 있는 동의 링크를 안내문으로 넣을 수 있습니다. 기술적인 파일 형식이나 업로드 형식을 몰라도 됩니다. 핵심은 "내가 어떤 의뢰인 질문에 잘 답할 수 있는 변호사인지"를 분명하게 정리하는 것입니다.
...
- JuriSupport를 쓰지 않기로 한 경우, 수정 가능한 개인 프로필 HTML 파일을 만듭니다.
...
11. JuriSupport에 올리지 않는다면 `jurisupport-personal-profile.html` 같은 수정 가능한 개인 프로필 HTML 파일을 만듭니다. 이 파일에는 원할 때 JuriSupport 프로필 검토 절차를 시작할 수 있는 동의 링크를 안내문으로 넣을 수 있습니다.
...
- JuriSupport에 올리지 않는 경우에는 로컬 HTML 파일만 만들며, 이 파일 자체가 공개나 업로드를 뜻하지 않습니다.
